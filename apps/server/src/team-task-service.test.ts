...
describe("TeamTaskService", () => {
  ...
  it("pauses the whole workflow when a specialist turn is denied the task's protected resource", async () => {
    ...
    expect(teamTasks.getTask(actor, task.id).lastError).toContain("GRANT_MISSING");
    ...
    const denial = teamTasks.getEvents(actor, task.id).find((event) => event.type === "resource_authorization");
